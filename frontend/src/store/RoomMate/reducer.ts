...
const reducer: Reducer<RoomMateState> = (state = {fetching: false, roomMates: [], status: ''}, action) => {
    switch (action.type) {
        ...
        case RoomMateType.ADD_ROOM_MATE_SUCCESS: {
            return {...state, fetching: false, roomMates: state.roomMates.concat(action.payload.data), status: 'success'}
        }
        case RoomMateType.ADD_ROOM_MATE_FAILURE: {
            return {...state, fetching: false, error: action.payload.error, status: 'failure'}
        }
