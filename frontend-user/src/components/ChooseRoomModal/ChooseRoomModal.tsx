...
import { addError, removeRoomMate } from "../../store/RoomMate/actions";
import { APIurl, enrollStudentsInRoom } from "../../store/api";

type ChooseRoomModalProps = {
  roomMates: RoomMate[];
  removeMate: (roomMate: RoomMate) => void;
  user: RoomMate;
...
const mapDispatchToProps = (
  ...
  return {
    removeMate: (roomMate: RoomMate) =>
      dispatch(removeRoomMate(roomMate.login)),
    addError: (message: string) => dispatch(addError(message))
  };
};

class ChooseRoomModal extends React.Component<
  WithStyles<typeof chooseRoomModalStyles> & ChooseRoomModalProps
> {
  state = {
    rooms: [],
  ...
  public componentDidMount() {
    const wb = new WebSocket(
      `ws://${APIurl}/ws/${this.props.user.event}/rooms/`
    );
    ...
    if (!roomMates.length) {
      ...
      return (
        ...
        >
          ...
          >
            <Paper className={classes.modalPaper} square={true}>
              <Typography variant="h5" id="modal-title">
                {`Czy na pewno? Chcesz mieszkać w pokoju ${
                  this.state.pickedRoom.number
                }?`}
              </Typography>
              <Typography variant="body1" id="simple-modal-description">
                {`Wybierasz ${
                  this.state.pickedRoom.capacity
                }-osobowy pokój nr ${this.state.pickedRoom.number}.
                            Po twojej rezerwacji jego stan zapełnienia zmieni się z ${
                              this.state.pickedRoom.occupancy
                            }/${this.state.pickedRoom.capacity}
                            do ${this.state.pickedRoom.occupancy +
                              this.props.roomMates.length +
                              1}/${this.state.pickedRoom.capacity}`}
              </Typography>
          ...
          <Paper className={classes.paper}>
            ...
            <Typography variant={"h5"}>Twoja grupa</Typography>
                ...
                return (
                  <Grid item={true} xs={12} sm={6} key={index}>
                    <UserChip
                      faculty={user.faculty}
                      name={user.name}
                      onDelete={() => this.props.removeMate(user)}
                    />
                  </Grid>
                );
              })}
            </Grid>
            <Typography variant={"h5"}>Wybierz pokój</Typography>
  ...
  private enrollStudentsInRoom = async () => {
    try {
      ...
      const result = await enrollStudentsInRoom(
        roomMates,
        pickedRoom.number,
        user.event
      );
      ...
      if (result.status === 200) {
        ...
        result.response && addError(result.response.data.detail);
  ...
  private updatePickedRoom = () => {
    ...
    if (
      this.props.roomMates.length >
      pickedRoom.capacity - pickedRoom.occupancy
    ) {
      this.setState({
        isModalVisible: false
      });
...
const ChooseRoomModalWithStyles = withStyles(chooseRoomModalStyles)(
  ChooseRoomModal
);
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withRouter(ChooseRoomModalWithStyles));
