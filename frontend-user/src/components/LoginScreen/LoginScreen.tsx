...
class LoginScreen extends React.Component<
  WithStyles<typeof loginScreenStyles> & Props
> {
  state = {
    username: "",
    password: ""
  ...
  componentWillMount() {
    this.validateIsLogged().then(isLogged => {
      if (isLogged) this.props.history.push("/AddingRoomMates");
    });
  }
      ...
      <div className={classes.container}>
        <Grid container={true} justify={"center"} alignItems={"center"}>
          <Grid item={true} xl={5} lg={6} md={8} xs={10}>
            <Paper square={true} className={classes.loginCard}>
              <FacultyLogo size={15} />
              ...
              <form onSubmit={this.tryAuthorize} className={classes.form}>
                ...
                <Button
                  ...
                  color="primary"
                >
                  <div className={classes.buttonLink}>Zaloguj</div>
                </Button>
              </form>
            </Paper>
          </Grid>
...
export default connect(
  null,
  mapDispatchToProps
)(withStyles(loginScreenStyles)(withRouter(LoginScreen)));
