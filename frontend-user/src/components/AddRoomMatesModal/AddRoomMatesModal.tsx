...
import {
  ...
  RoomMate,
  StudentErrors
} from "../../store/RoomMate/types";
import {
  addError,
  initFetchRoomMate,
  removeRoomMate
} from "../../store/RoomMate/actions";
...
): Partial<AddRoomMatesModalProps> => {
  return {
...
const mapDispatchToProps = (
  ...
  return {
    initFetchRoomMate: (login, eventName) =>
      initFetchRoomMate(login, eventName)(dispatch),
    removeRoomMate: login => dispatch(removeRoomMate(login)),
    sendSignedUserAddedError: (name: string) =>
      dispatch(addError(StudentErrors.addedYet(name))),
        ...
        >
          <Paper className={classes.paper}>
            ...
            <Typography
              ...
              color={"inherit"}
            >
              Jeśli chcesz być w pokoju z wybranymi osobami, potrzebujemy waszej
              obupólnej zgody. Dodaj do grupy swoich znajomych za pomocy kodów
              identyfikujących, które zostały wysłane na wasze maile. Wybierz
              opcję "jestem sam" jeśli nie ma dla Ciebie różnicy z kim będziesz
              w pokoju, albo po prostu chcesz poznać nowych ludzi.
            </Typography>
            <TextField
              variant={"standard"}
              label={"Kod"}
            ...
            <Grid container={true} className={classes.userChipsContainer}>
              {roomMates.map((roomMate: RoomMate, index: number) => (
                <Grid key={index} item={true} sm={6}>
                  <UserChip
                    key={index}
                    onDelete={() => removeRoomMate(roomMate.login)}
  ...
  private handleClickEnrollAlone = async () => {
    try {
      ...
      if (result.status === 200) {
        ...
        resultBody && addError(resultBody.details);
      }
    } catch (e) {
      throw e;
    }
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withStyles(addRoomMatesModalStyles)(withRouter(AddRoomMatesModal)));
